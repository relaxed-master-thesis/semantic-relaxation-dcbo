...
//ad create thread elimination array pointer
volatile exchanger_t *elimination_array;
volatile wait_counter_t *wait_array;
volatile uint32_t push_wait=300;
volatile uint32_t pop_wait=1;
...
volatile ticks *elimination_succ;
volatile ticks *elimination_fail;
volatile ticks *removing_succ;
volatile ticks *removing_fail;
volatile ticks *putting_count;
volatile ticks *putting_count_succ;
volatile ticks *elimination_count;
volatile ticks *elimination_count_succ;
volatile ticks *elimination_bounce_count;
volatile ticks *stack_bounce_count;
...
int main(int argc, char **argv)
{
  ...
  /* printf("update = %f (putting = %f)\n", update_rate, put_rate); */
	...
	elimination_succ = (ticks *) calloc(num_threads , sizeof(ticks));
	elimination_fail = (ticks *) calloc(num_threads , sizeof(ticks));
	...
	elimination_count = (ticks *) calloc(num_threads , sizeof(ticks));
	elimination_count_succ = (ticks *) calloc(num_threads , sizeof(ticks));
	elimination_bounce_count = (ticks *) calloc(num_threads , sizeof(ticks));
	stack_bounce_count = (ticks *) calloc(num_threads , sizeof(ticks));
	...
	//ad initialize elimination array with cache alignement to CACHE_LINE_SIZE to avoid false sharing
	elimination_array = (exchanger_t *) calloc(num_threads , sizeof(exchanger_t));//push array
	wait_array = (wait_counter_t *) calloc(num_threads , sizeof(wait_counter_t));//wait array counter for pause
	push_wait=push_wait*num_threads;
    pop_wait=pop_wait*num_threads;
	...
	volatile ticks elimination_suc_total = 0;
	volatile ticks elimination_fal_total = 0;
	...
	volatile uint64_t elimination_count_total = 0;
	volatile uint64_t elimination_count_total_succ = 0;
	volatile uint64_t elimination_bounce_count_total = 0;
	volatile uint64_t stack_bounce_count_total = 0;
	...
	for(t=0; t < num_threads; t++)
    {
		...
		elimination_suc_total += elimination_succ[t];
		elimination_fal_total += elimination_fail[t];
		...
		elimination_count_total += elimination_count[t];
		elimination_count_total_succ += elimination_count_succ[t];
		elimination_bounce_count_total += elimination_bounce_count[t];
		stack_bounce_count_total += stack_bounce_count[t];
		...
		long unsigned get_suc = (elimination_count_total_succ) ? elimination_suc_total / elimination_count_total_succ : 0;
		long unsigned get_fal = (elimination_count_total - elimination_count_total_succ) ? elimination_fal_total / (elimination_count_total - elimination_count_total_succ) : 0;
	...
	double elimination_perc_succ = (1 - (double) ((elimination_count_total + elimination_bounce_count_total) - elimination_count_total_succ) / elimination_count_total) * 100;
	...
	//printf("elimination_count_total , %-10llu \n", (LLU) elimination_count_total);
	//printf("elimination_count_total_succ , %-10llu \n", (LLU) elimination_count_total_succ);
	//printf("elimination_perc_succ , %10.1f \n", elimination_perc_succ);
	//printf("elimination_per_second , %10.1f \n", (elimination_count_total_succ * 1000.0 / duration));
	...
	#if GC == 1
		ssmem_free(alloc, (void*) elimination_array);
	#endif
