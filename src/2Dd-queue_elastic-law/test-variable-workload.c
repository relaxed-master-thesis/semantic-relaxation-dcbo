...
{
	// return id > 0;
	// return id < (num_threads*3)/4;
	// return id < (num_threads)/4;
	// return id < num_threads/2;
	// return id < num_threads - 2;
	// return id < (num_threads*85)/100;
...
void* test(void* thread)
{
	...
	#define MAX_TIMESTAMPS 3.2e6
 	...
 	uint64_t *my_widths = calloc(MAX_TIMESTAMPS, sizeof(uint64_t));
	size_t timeit = 0;
	#ifdef RELAXATION_ANALYSIS
	...
	for (int thread = 0; thread < num_threads; thread++) {
		for (int i = 0; timestamps[thread][i] != 0; i += 1) {
				if (is_producer(thread))
					printf("[%u] prod timestamp %d: %zu ns, %zu width\n", thread, i, timestamps[thread][i], widths[thread][i]);
				else
					printf("[%u] cons timestamp %d: %zu ns, %zu width\n", thread, i, timestamps[thread][i], widths[thread][i]);
		}
	}
	#else
