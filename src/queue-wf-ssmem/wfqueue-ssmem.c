...
/* ADDED FOR D-CHOICE QUEUE */

extern __thread int thread_id;
__thread ssmem_allocator_t* alloc;
__thread handle_t thread_handle;
// extern __thread handle_t thread_handle;

// Needed for some init functions
extern size_t num_threads;

/* End of added for d-choice */
...
void wfqueue_init(queue_t *q, int nprocs, int thread_id) {
	#if GC == 1
    if (alloc == NULL)
    {
...
queue_t* wfqueue_create(int nprocs, int thread_id)
{
    ...
    wfqueue_init(wfqueue, nprocs, thread_id);
    return wfqueue;
}
...
// Allocates a handle, initiating the GC if not already created
handle_t* wfqueue_register(queue_t *q, int id) {
    ...
    // Cannot alloc with ssmem, as it requires the same size for all objects
    handle_t* th = aligned_alloc(CACHE_LINE_SIZE, sizeof(*th));
